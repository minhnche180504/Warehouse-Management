...
<html lang="en">
    <head>
        ...
        <link rel="canonical" href="importRequestList" />
                ...
                <main class="content">
                    <div class="container-fluid p-0">
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <h1 class="h3 fw-bold">Import Request List</h1>
                            <a href="requestImport" class="btn btn-primary">
                                <i class="align-middle" data-feather="plus-circle"></i>Create Import Request
                            </a>
                        </div>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-12">
                                <form action="${pageContext.request.contextPath}/sale/importRequestList" method="GET" class="row g-3">
                                    <div class="col-md-4">
                                        ...
                                        <table id="datatables-import-requests" class="table table-striped table-hover" style="width:100%">
                                            ...
                                            <tbody>
                                                <c:forEach items="${importRequests}" var="ir">
                                                    <tr>
                                                        <td>IR-<fmt:formatNumber value="${ir.requestId}" pattern="000"/></td>
                                                        <td><fmt:formatDate value="${ir.requestDate}" pattern="yyyy-MM-dd"/></td>
                                                        <td>${ir.createdByName}</td>
                                                        <td>
                                                            <c:choose>
                                                                <c:when test="${not empty ir.details}">
                                                                    <c:forEach var="detail" items="${ir.details}" varStatus="status">
                                                                        ${detail.productName} (x${detail.quantity})<c:if test="${!status.last}">, </c:if>
                                                                    </c:forEach>
                                                                </c:when>
                                                        ...
                                                        <td>
                                                            ...
                                                            <td class="action-btns">
                                                                <a href="importRequestViewDetail?id=${ir.requestId}" class="btn btn-sm btn-info me-1" title="View Details">
                                                                    <i data-feather="eye"></i>
                                                                </a>
                                                                <c:if test="${ir.status == 'Pending'}">
                                                                    <a href="importRequestEdit?id=${ir.requestId}" class="btn btn-sm btn-warning" title="Edit">
